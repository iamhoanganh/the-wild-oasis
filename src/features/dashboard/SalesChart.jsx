...
function SalesChart ({ bookings, numDays, }) {
  ...
  const data = allDates.map((date) => {
    return {
      ...
      totalSales: bookings.filter(booking => isSameDay(date, new Date(booking.created_at))).reduce((acc, current) => acc + current.totalPrice, 0),
      extrasSales: bookings.filter(booking => isSameDay(date, new Date(booking.created_at))).reduce((acc, current) => acc + current.extrasPrice, 0),
    }
  })
  ...
  return (
    <StyledSalesChart>
      <Heading as={'h2'}>Sales from {format(allDates.at(0), "dd/MM/yyyy")} &mdash; {format(allDates.at(-1), "dd/MM/yyyy")}</Heading>
